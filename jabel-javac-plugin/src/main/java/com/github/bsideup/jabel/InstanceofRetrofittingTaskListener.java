 ...
 * <li>Nested record patterns (if obj instanceof Line(Point(int x1, int y1),
 * Point end)).</li>
...
public class InstanceofRetrofittingTaskListener implements TaskListener {
    ...
    public class InstanceofTranslator extends TreeTranslator {
        ...
        public <T extends JCTree> T translate(T tree) {
            ...
            if (tree instanceof JCIf) {
                ...
                if (!(cond instanceof JCInstanceOf)) {
    ...
    public JCExpression unwrapParenthesis(JCExpression expr) {
        while (expr instanceof JCParens) {
     ...
     * Into:
     * {@code if (obj instanceof Point) { Point $rec = (Point)obj; int x = $rec.x(); ... body } }
     */
    ...
    public JCBlock buildBlock(List<JCVariableDecl> declarations, JCStatement body) {
        ...
        for (JCVariableDecl decl : declarations) {
            stmts.append(decl);
        }
        if (body instanceof JCBlock) {
            for (JCStatement stmt : ((JCBlock) body).stats) {
                stmts.append(stmt);
            }
        } else if (body != null) {
            stmts.append(body);
