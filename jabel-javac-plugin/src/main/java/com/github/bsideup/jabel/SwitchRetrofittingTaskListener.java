...
public class SwitchRetrofittingTaskListener implements TaskListener {
    ...
    // Some internal states to avoid getting errors everytimes we trying to use a
    // not found feature.
    ...
    private static JCExpression getGuard(JCCase caseTree) {
        if (GUARDS != null) {
            return GUARDS ? caseTree.getGuard() : null;
        }
        try {
    ...
    private static List<JCTree> getLabels(JCCase caseTree) {
        ...
        if (LABELS) {
            return (List<JCTree>) (List<?>) caseTree.getLabels();
        }
    ...
    private static JCTree getBody(JCCase caseTree) {
        if (BODIES != null) {
            return BODIES ? caseTree.getBody() : null;
        }
        try {
    ...
    private JCCase makeCase(
            CaseTree.CaseKind kind, List<? extends JCTree> labels, JCExpression guard,
    ...
    // Because the return type of these two methods may not exist, we need delay the
    // call to an inner class.
    ...
    private JCTree makeDefaultCaseLabel() {
        if (DEFAULT_CASES != null) {
            return DEFAULT_CASES ? DefaultCaseLabelFactory.make(make) : null;
        }
        try {
    ...
    private static JCExpression getLabelExpression(JCTree label) {
        if (label instanceof JCExpression) {
            return (JCExpression) label;
        }
        if (isConstant(label)) {
            return ((JCConstantCaseLabel) label).expr;
        }
        return null;
    }
    ...
    /**
     * Replaces a pattern/null switch to a standard switch using a ternary-chain
     * dispatcher in the condition.
     * <p>
     * The ternary chain maps each case condition to an index which becomes the new
     * selector. <br>
     * Each case label is rewritten with this index, plus the record pattern, if
     * any, is lowered to the case. <br>
     *
    ...
    public List<JCStatement> buildCaseBody(JCCase c, JCExpression sel, boolean expression) {
        ...
        if (body instanceof JCBlock) {
            for (JCStatement s : ((JCBlock) body).stats) {
                out.append(s);
            }
        } else if (body != null) {
    ...
    /**
     * Injects
     * {@code default: throw new UnsupportedOperationException("MatchException");}
    ...
    public JCExpression buildLabelCondition(JCTree label, JCExpression sel) {
        if (isNull(label)) {
            ...
            if (pv != null) {
                return make.TypeTest(helper.copy(sel), pv.vartype);
            }
            JCExpression rt = getRecordType(label);
            if (rt != null) {
                return make.TypeTest(helper.copy(sel), rt);
            }
            JCExpression pt = getPatternType(label);
            if (pt != null) {
                return make.TypeTest(helper.copy(sel), pt);
            }
        ...
        for (JCTree label : labels) {
            ...
            for (JCTree np : nested) {
                ...
                map.put(nv.name, make.Apply(
                        List.nil(),
                        make.Select(
                                make.TypeCast(rt, helper.copy(sel)),
                                cn != null ? cn : nv.name
                        ),
                        List.nil()
               ));
    ...
    private boolean isComplex(JCExpression expr) {
        ...
        if (expr instanceof JCFieldAccess) {
            return isComplex(((JCFieldAccess) expr).selected);
        }
        if (expr instanceof JCParens) {
            return isComplex(((JCParens) expr).expr);
        }
    ...
    private JCExpression extractExpression(JCTree body) {
        if (body instanceof JCExpressionStatement) {
