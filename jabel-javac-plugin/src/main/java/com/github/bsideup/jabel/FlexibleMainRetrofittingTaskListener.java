 ...
 * ({@code void main();, void main(String[]);, static void main();,
 * static void main(String[])}) by attempting to create a bridge.
 * <p>
 * This is pretty limited as non-static main entry points with arguments cannot
 * be adapted, due to a signature duplication with the static one.
 * <p>
 * Moreover, classes containing multiple entry points with ones that are
 * private, will not work properly on older JVMs. <br>
 ...
 * }</pre>
 *
 * The first main method will be selected by Java 25's JVM, because the standard
 * one is private, whereas on an older JVM, an error will be displayed. <br>
 * And Jabel cannot create a bridge for the first main, because there would be a
 * signature duplication with the second declared main.
 */
        ...
        for (JCTree def : classDecl.defs) {
            ...
            if (!isMain(method)) {
                if (method.name == names.init) {
                    hasConstructor = true;
                    if (!isPrivate(method) && method.params.isEmpty()) {
                        canInstanciate = true;
                    }
                }
                continue;
    ...
    public JCMethodDecl makeMainBridge(JCClassDecl classDecl, boolean toLocalMain) {
        return make.MethodDef(
                ...
                List.of(make.VarDef(
                        ...
                        make.TypeArray(make.Type(syms.stringType)),
                        null
                )),
                List.nil(),
